...
void creatore_sottoreti_c_host(int *ip_dec_dot);
void creatore_sottoreti_b_host(int *ip_dec_dot);
void creatore_sottoreti_a_host(int *ip_dec_dot);
...
void creatore_sottoreti_a_host(int *ip_dec_dot){
  ...
  int intervallo;
  int ip_temp[32];
  clone_ip_dec_dot[0] = ip_dec_dot[0];
  clone_ip_dec_dot[1] = ip_dec_dot[1];
  clone_ip_dec_dot[2] = ip_dec_dot[2];
  clone_ip_dec_dot[3] = ip_dec_dot[3];
  for(i = 0; i < 32; i++){
    ip.subnet_mask_bin[i] = 0;
  }
  pot2 = 0;
  ...
  intervallo = 256 / pot2;
  ip.host_per_sottorete = 0;
  ip.host_per_sottorete = pow(2, (32 - (8 + (cont_pot - 1))));

  do{
    printf("Inserisci gli host per sottorete (MAX %d)\n", ip.host_per_sottorete);
    scanf("%d", &ip.host);
    if(ip.host <= 0 || ip.host > ip.host_per_sottorete){
      printf("Non è possibile soddisfare la sua richiesta\n");
    }
  }while(ip.host <= 0 || ip.host > ip.host_per_sottorete);
  for(i = 0; i < 4; i++){
    clone_ip_dec_dot[i] = ip_dec_dot[i];
  }
  printf("\tNETWORK ID\tGATEWAY\t\tBROADCAST\tPRIMO HOST\tULTIMO HOST\n");
  ...
  printf("INDIRIZZO IP CON CIDR: %d.0.0.0/%d\nHost per sottorete: %d\nSubnet mask: %d.%d.%d.%d\n", clone_ip_dec_dot[0], 8 + (cont_pot - 1), ip.host, ip.subnet_mask_dec[0], ip.subnet_mask_dec[1], ip.subnet_mask_dec[2], ip.subnet_mask_dec[3]);
...
void creatore_sottoreti_b_host(int *ip_dec_dot){
  ...
  ip.host_per_sottorete = 0;
  ip.host_per_sottorete = pow(2, (32 - (16 + (cont_pot - 1))));
  ...
  conversione_binario_decimale(ip.subnet_mask_dec, ip.subnet_mask_bin);
  printf("INDIRIZZO IP CON CIDR: %d.0.0.0/%d\nSubnet mask: %d.%d.%d.%d\n", clone_ip_dec_dot[0], 16 + (cont_pot - 1), ip.subnet_mask_dec[0], ip.subnet_mask_dec[1], ip.subnet_mask_dec[2], ip.subnet_mask_dec[3]);
}
...
void creatore_sottoreti_c_host(int *ip_dec_dot){
  ...
  ip.host_per_sottorete = 0;
  ip.host_per_sottorete = pow(2, (32 - (24 + (cont_pot - 1))));
  do{
    printf("Inserisci gli host per sottorete (MAX %d)\n", ip.host_per_sottorete);
    scanf("%d", &ip.host);
    if(ip.host <= 0 || ip.host > ip.host_per_sottorete){
  ...
  conversione_binario_decimale(ip.subnet_mask_dec, ip.subnet_mask_bin);
  printf("INDIRIZZO IP CON CIDR: %d.0.0.0/%d\nHost per sottorete: %d\nSubnet mask: %d.%d.%d.%d\n", clone_ip_dec_dot[0], 24 + (cont_pot - 1), ip.host, ip.subnet_mask_dec[0], ip.subnet_mask_dec[1], ip.subnet_mask_dec[2], ip.subnet_mask_dec[3]);
